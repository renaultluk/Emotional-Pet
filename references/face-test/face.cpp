...
void Face::changeFaceState(faceState_t newFaceState)
{
  ...
  switch (newFaceState)
  {
    ...
    case SAD:
      ...
      rightEye.addKeyframe(175, 94, 37, 104, 2.0);
      faceBottom.addKeyframe(36, 203, 204, 63, 2.0);
      velocityFunc = easeInOut;
      // anim_time = 2 * FRAME_RATE;
      break;
    ...
    case BLINK:
      ...
      forehead.addKeyframe(snapshot.x, snapshot.y, snapshot.w, snapshot.h + 120, 0.5, true);
      forehead.addKeyframe(snapshot.x, snapshot.y, snapshot.w, snapshot.h, 0.5, true);
      velocityFunc = linear;
